...
public class SimpleCarController : MonoBehaviour {
    ...
    private void Audio()
    {
        var valPitch = Speed / MaxSpeed + 1.5f;
        GetComponent<AudioSource>().pitch = Mathf.Clamp(valPitch, 1f, 3f);
    }
    ...
    private void Direction()
    {
        if (_rightArrow || _leftArrow)
        {
            ...
            Debug.Log(DA);
            var steerAngle = Input.GetAxis("Horizontal") * DA;
    ...
    private void Drift()
    {
        ...
        {
            _drift = true;
            frictionBL.stiffness = 0.8f;          
            frictionBR.stiffness = 0.8f;
